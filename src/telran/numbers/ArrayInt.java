...
public class ArrayInt {
	...
	public static int[] addNumber(int ar[], int number) {
		int[] localArr = Arrays.copyOf(ar, ar.length + 1);
		localArr[localArr.length - 1] = number;
		return localArr;
	}
	...
	public static int[] insertNumber(int ar[], int index, int number) {
		int[] localArr = Arrays.copyOf(ar, ar.length + 1);
		System.arraycopy(ar, index, localArr, index + 1, ar.length - index);
		localArr[index] = number;
		return localArr;
